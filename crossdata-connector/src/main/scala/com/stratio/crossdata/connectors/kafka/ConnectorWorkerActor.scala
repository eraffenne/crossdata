...
class ConnectorWorkerActor(connector: IConnector, metadataMapAgent: Agent[ObservableMap[Name, UpdatableMetadata]], runningJobs:  Agent[mutable.ListMap[String, ActorRef]]) extends Actor with ActorLogging with IResultHandler {
  ...
  override def receive: Receive = {
    ...
    case StartKafka(connectorName) => {
      ...
      val kafkaConsumer = new KafkaConsumer(connectorName)
      for ( messageAndMetadata <- kafkaConsumer.stream ){
         logger.info("offset received "+messageAndMetadata.offset +"and fetched ->"+messageAndMetadata)
         consumeKafka(messageAndMetadata.key(), messageAndMetadata.message())
      }
    }
  ...
  private def consumeKafka(clusterCatalogTableString: String, message: String): Unit ={
    val clusterCatalogTable = clusterCatalogTableString.split("\\.")
    val clusterName = new ClusterName(clusterCatalogTable(0))
    val tableName= new TableName(clusterCatalogTable(1),clusterCatalogTable(2))
    val tableMetadata = getTableMetadata(clusterName, tableName)

    try {
      val result = connector.getStorageEngine.insertBatch(clusterName, tableMetadata, util.Arrays.asList(util.Arrays.asList(message.split("\\."))), false)
      logger.info("Consuming KeyedMessage[key, value]: [" + clusterCatalogTableString + "," + message + "]")
    } catch {
      case e: Exception => {
        logger.error("Exception consuming from Kafka: [" + e.getMessage +"]")
      }
      case err: Error =>
        logger.error("Error consuming from Kafka: [" + err.getMessage +"]")
    }
  }
