...
class KafkaConnectorApp extends ConnectConfig with IConnectorApp {

  override lazy val logger = Logger.getLogger(classOf[KafkaConnectorApp])
 ...
 */
  ...
  implicit val stTimeout = Timeout(FiniteDuration(KafkaConnectorApp.GetStatusTimeout,TimeUnit.SECONDS))
    connectorActor.map[ConnectionStatus]{ cActor =>
      val future = (cActor ? GetConnectorStatus).mapTo[Boolean]
      Try(Await.result(future,FiniteDuration.apply(KafkaConnectorApp.GetStatusTimeout, TimeUnit.SECONDS))).map{ isConnected =>
